...
public class MyFirstCoprocessExample {
     ...
     * @return 返回表的总条数
     */
    public static long execFastEndpointCoprocessor(Table table, final String family, final String columns) {
        long start_t = System.currentTimeMillis();
        //定义总的 rowCount 变量
        final AtomicLong totalRowCount = new AtomicLong();
        final AtomicDouble maxValue = new AtomicDouble(Double.MIN_VALUE);
        final AtomicDouble minValue = new AtomicDouble(Double.MAX_VALUE);
        final AtomicDouble sumValue = new AtomicDouble();
